...
loadFixtures().catch((error) => {
  gallery.replaceChildren(renderMessage(error instanceof Error ? error.message : "Failed to load browser fixture gallery."));
});

async function loadFixtures() {
  const payload = await loadFixtureManifest();
  const fixtures = Array.isArray(payload.fixtures) ? payload.fixtures : [];

  if (fixtures.length === 0) {
    gallery.replaceChildren(renderMessage("No exported browser fixture JSON files were found in out-web/."));
    return;
  }
...
async function loadFixtureManifest() {
  const staticResponse = await fetch("./fixtures-web-manifest.json");
  if (staticResponse.ok) {
    return await staticResponse.json();
  }

  const apiResponse = await fetch("/api/fixtures-web");
  if (!apiResponse.ok) {
    throw new Error("Failed to load browser fixture list.");
  }
